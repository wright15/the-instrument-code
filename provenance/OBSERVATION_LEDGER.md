...
## OBS-022 — D5 contact geometry derivation

**Status:** derived `planning_evidence` 2026-09-12
**Scope:** canonical D5 `SEAT_CONTACT` geometry at registered Z7 office-coordinate resolution only

**Provenance:** Maintainer independent re-audit closed and approved the single-pass
GOV-517 generator receipt. The pure engine generated 10 T1 witnesses and 28 T2
witnesses from registered I1-I4 inputs, then the downstream G5 comparison mapped
them to all 14 canonical D5 contacts after generation was sealed. T1 covers all
14 contacts through its 10 office-pair witnesses; T2 covers all 14 contacts
through its 28 construction-edge witnesses. The exact G5 distribution is
`both: 14, primary-only: 0, secondary-only: 0, neither: 0`; T3 converges at all
seven offices. Every canonical D5 contact's office coordinate is
convergence-generated by both routes, reproducing the canonical contact geometry
exactly at office-coordinate resolution.

**Frame-level combinatorics guard:** this verifies only the registered
frame-level witness counts, office-coordinate coverage, and convergence map. It
asserts no hypothesis meaning or disposition; those effects are recorded only in
the GOV-517 decision entry.

**Binding philosophy:** I1-I4 input channels remain isolated from observed D5
outcomes during generation, and G1-G5 perform blind generation before the
sealed output reaches the canonical comparison layer.

**Falsification:** any re-run under unchanged bindings producing different
attributions, witness counts, office-convergence counts, or G5 cell distribution
invalidates this entry.

**Upstream:** `qa/gov-517-canonical-derivation-report.json` (SHA-256
`1b0432c9426d5cdd1fabb01a073967bc5c9ccee2166c07c03e36e5359bbdbb0e`),
`scripts/gov-517-d5-derivation-engine.mjs`, OBS-008, OBS-009, and
