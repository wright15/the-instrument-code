...
**R2 — Constraint set.** `C_adj(X): for every i in Z_7, x_i != x_{i+1}` (registered office-ring adjacency axiom). `C_close(X): sum_{i in Z_7} x_i = 0 in Z_7` (7-cycle closure and coordinate consistency). `C(X) = C_adj(X) and C_close(X)`. Each predicate is invariant under cyclic shift and reflection. **Amendment record:** `C_step2` (distance-2 K construction-step; OBS-008) was removed by "GOV-518 degeneracy ruling and boundary amendment — 2026-09-07" — as originally registered, `C_step2` is the identical predicate whose truth the derivation map's mask `b_i` registers, forcing the statistic constant on the admissible set (degenerate-by-construction; proof in the amendment entry). The distance-2 comparison's OBS-008 provenance is retained in R1's derivation-map role. **Selection attestation (amended):** the removal is justified by the degeneracy arithmetic alone; no replacement comparison is introduced; no constraint or statistic was selected by reference to any R7-excluded input or by effect on the outcome space; OBS-004/005/009 and the D-signature exclusion authority are not K-exhaustivity sources. Provenance: `provenance/DECISION_LEDGER.md`, "GOV-518 maintainer mathematical registration — 2026-09-07"; "GOV-518 degeneracy ruling and boundary amendment — 2026-09-07".
...
| D6 | Constraint-level witness `X_wit = (0,0,0,0,0,0,0)` (fails C_adj; satisfies C_close); statistic-level witnesses `X_a = (0,1,2,3,4,5,6)` and `X_b = (2,0,2,1,2,3,4)` (both C-admissible; `f(X_a) = (7,7,7,7,7,7,7)`, `f(X_b) = (1,0,1,0,4,3,2)` — different stat-orbits); per-constraint strictness: C_adj strict per the zeros witness, C_close strict per `(0,1,0,1,0,1,2)` (adjacency-true, sum 5 ≠ 0 in Z_7) | two-level non-vacuity demonstrated; witnesses pass R7 screen | Registered; see `DECISION_LEDGER.md`, "GOV-518 degeneracy ruling and boundary amendment — 2026-09-07" |
